...
class Vector {
 public:
  ...
  /// getters and setters
  Point<T, dim>& data() noexcept { return point_; }
  const Point<T, dim>& data() const noexcept { return point_; }
  T& operator[](size_t index) { return point_.cords[index]; }
  const T& operator[](size_t index) const { return point_.cords[index]; }
  T& Cord(size_t index);
  const T& Cord(size_t index) const;
 ...
 private:
  void PushCord(size_t index, const T& value);
  template <typename... Args>
  void PushCord(size_t index, const T& value, Args ...args);
...
template <typename T, size_t dim>
T SquaredDistance(const Vector<T, dim>& a, const Vector<T, dim>& b);

template <typename T, size_t dim>
T Distance(const Vector<T, dim>& a, const Vector<T, dim>& b);
...
template <typename T, size_t dim>
Vector<T, dim>::Vector(const Point<T, dim>& data): point_(data) {}

template <typename T, size_t dim>
Vector<T, dim>::Vector(const std::vector<T>& data) {
...
template <typename T, size_t dim>
Vector<T, dim>::operator Point<T, dim>() {
  Point<T, dim> result = point_;
  return result;
}
...
template <typename T, size_t dim>
T& Vector<T, dim>::Cord(size_t index) {
  if (index >= dim) {
...
template <typename T, size_t dim>
const T& Vector<T, dim>::Cord(size_t index) const {
  if (index >= dim) {
    throw std::out_of_range("bad index");
  }
  return operator[](index);
...
template <typename T, size_t dim>
void Vector<T, dim>::PushCord(size_t index, const T& value) {
  point_.cords[index] = value;
}

template <typename T, size_t dim>
template <typename... Args>
void Vector<T, dim>::PushCord(size_t index, const T& value, Args... args) {
  point_.cords[index] = value;
  PushCord(index + 1, args...);
}
